...
class TypeNetMLFeatureExtractor:
    ...
    def __init__(
        self,
        data_path: str = "typenet_features_extracted.csv",
        ...
        if not self.keep_outliers:
            self.df = self.df[~self.df["outlier"]].copy()
        ...
        print(
            f"Sessions per platform: {self.df.groupby('platform_id')['session_id'].nunique().to_dict()}"
        )
        ...
        for unigram in unigrams:
            ...
            if len(unigram_data) > 0:
                ...
                features[f"HL_{unigram}_std"] = (
                    unigram_data.std() if len(unigram_data) > 1 else 0
                )
        ...
        for digram in digrams:
            ...
            if len(digram_data) > 0:
                ...
                features[f"IL_{digram}_std"] = (
                    digram_data.std() if len(digram_data) > 1 else 0
                )
    ...
    def create_dataset_1(
        self, unigrams: List[str], digrams: List[str], imputation: str = "global"
    ) -> pd.DataFrame:
        """
        ...
        for (user_id, platform_id), group_data in self.df.groupby(
            ["user_id", "platform_id"]
        ):
    ...
    def create_dataset_2(
        ...
        for (user_id, platform_id, session_id), group_data in self.df.groupby(
    ...
    def create_dataset_3(
        ...
        for (user_id, platform_id, session_id, video_id), group_data in self.df.groupby(
    ...
    def apply_imputation(
        self, dataset: pd.DataFrame, strategy: str, level: str
    ) -> pd.DataFrame:
        """
    ...
    def create_experiment_splits(
        self, dataset: pd.DataFrame, experiment_type: str, experiment_config: Dict
    ...
    def generate_all_experiments(self, output_dir: str = "ml_experiments"):
        ...
        for imputation in ["global", "user"]:
            ...
            for dataset_name, dataset in datasets.items():
                if self.keep_outliers:
                    print(f"Saving full {dataset_name} with outliers")
                    dataset.to_csv(
                        imp_dir / f"{dataset_name}_full_with_outliers.csv", index=False
                    )
                else:
                    print(f"Saving full {dataset_name} without outliers")
                    dataset.to_csv(
                        imp_dir / f"{dataset_name}_full_without_outliers.csv",
                        index=False,
                    )
            ...
            #             print(
            #                 f"  Skipping {dataset_name} - no session information at platform level"
            #             )
            #             continue
            ...
            #             train_data.to_csv(
            #                 exp_dir / f"{dataset_name}_train.csv", index=False
            #             )
            #             test_data.to_csv(
            #                 exp_dir / f"{dataset_name}_test.csv", index=False
            #             )

            #             print(
            #                 f"  {dataset_name}: Train shape {train_data.shape}, Test shape {test_data.shape}"
            #             )
        ...
        print(
            f"Dataset types: 3 (user-platform, user-platform-session, user-platform-session-video)"
        )
