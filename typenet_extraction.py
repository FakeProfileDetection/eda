...
class TypeNetFeatureExtractor:
    ...
    def match_press_release_pairs(self, df: pd.DataFrame) -> List[Dict]:
        ...
        events.sort(
            key=lambda x: (
                x["press_time"] if x["press_time"] is not None else x["release_time"]
            )
        )
    ...
    def calculate_features(self, key1_event: Dict, key2_event: Dict) -> Dict:
        ...
        try:
            if (
                key1_event["press_time"] is not None
                and key1_event["release_time"] is not None
            ):
        ...
        if key1_event["valid"] and key2_event["valid"]:
            try:
                ...
                if (
                    key1_event["release_time"] is not None
                    and key2_event["press_time"] is not None
                ):
                    features["IL"] = (
                        key2_event["press_time"] - key1_event["release_time"]
                    )
                ...
                if (
                    key1_event["release_time"] is not None
                    and key2_event["release_time"] is not None
                ):
                    features["RL"] = (
                        key2_event["release_time"] - key1_event["release_time"]
                    )
            ...
            features.update(
                {
                    ...
                    "sequence_id": i,
                    "key1_timestamp": (
                        key1_event["press_time"]
                        if key1_event["press_time"] is not None
                        else key1_event["release_time"]
                    ),
                }
            )
    ...
    def process_dataset(
        self, root_dir: str, output_file: str = "typenet_features_extracted.csv"
    ):
        """
        ...
        if all_user_features:
            ...
            # Print error summary
            print("\nError summary:")
            error_counts = final_df[~final_df["valid"]][
                "error_description"
            ].value_counts()
